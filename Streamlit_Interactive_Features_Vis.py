...
campaign_name_filter = st.selectbox('Select Campaign', options=['All'] + control_group['Campaign'].unique().tolist())
...
if campaign_name_filter != 'All':
    filtered_control_group = filtered_control_group[filtered_control_group['Campaign'] == campaign_name_filter]
    filtered_test_group = filtered_test_group[filtered_test_group['Campaign'] == campaign_name_filter]
...
box_plot_spend.add_trace(go.Box(x=filtered_control_group['Campaign'], y=filtered_control_group['Spend [USD]'], name='Control Campaign', boxpoints='all'))
box_plot_spend.add_trace(go.Box(x=filtered_test_group['Campaign'], y=filtered_test_group['Spend [USD]'], name='Test Campaign', boxpoints='all'))

box_plot_spend.update_layout(title='Spend Comparison - Control vs. Test Campaign', xaxis_title='Campaign', yaxis_title='Spend [USD]')
